...
class AdjustGamma:
    def __init__(self, gamma=1.0, gain=1.0):
        """
        Args:
            gamma (float): Non-negative real number. Values > 1 will darken the image,
                           values < 1 will lighten the image.
            gain (float): The constant multiplier applied after adjusting gamma.
        """
        self.gamma = gamma
        self.gain = gain
...
class DarkLightDataset(torch.utils.data.Dataset):
    def __init__(self, root, light_transform, dark_transform, gamma=2.2):
    ...
    def __getitem__(self, index):
        ...
        light_image = self.light_transform(image)
        dark_image = self.dark_transform(image)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    class Decoder(nn.Module):
        ...
        def __init__(
            self, in_channels, out_channels, exit_channels=3, padding=1, uphill=5
        ):
            ...
            self.final_layer = nn.Conv2d(
                64, self.exit_channels, kernel_size=3, stride=1, padding=1
            )
    ...
    if args.dummy:
        ...
        train_dark_transforms = transforms.Compose(
            [
        ...
        train_light_transforms = transforms.Compose(
            [
                transforms.RandomResizedCrop(224),
                transforms.RandomHorizontalFlip(),
                transforms.ToTensor(),
        ...
        val_dark_transforms = transforms.Compose(
            [
        ...
        val_light_transforms = transforms.Compose(
            [
                transforms.Resize(256),
                transforms.CenterCrop(224),
                transforms.ToTensor(),
        ...
        # Create the dataset
        train_dataset = DarkLightDataset(
            root=traindir,
            light_transform=train_light_transforms,
            dark_transform=train_dark_transforms,
        )

        val_dataset = DarkLightDataset(
            root=valdir,
            light_transform=val_light_transforms,
            dark_transform=val_dark_transforms,
        )
...
def train(
    ...
    for i, (dark_images, light_images, target) in enumerate(train_loader):
        ...
        loss_CLS.update(
            loss["cls"].item(), dark_images.size(0)
        )  # TO DO : add MSE loss to the AverageMeter
        loss_MSE.update(
            loss["mse"].item(), dark_images.size(0)
        )  # TO DO : add MSE loss to the AverageMeter
        ...
        # loss['mse'].backward()
        sum(
            loss[k] for k in loss
        ).backward()  # TO DO : add regularization parameter for balance between classification and reconstruction loss
        optimizer.step()
...
def validate(val_loader, model, criterion, device, args, log_file=None):
    ...
    def run_validate(loader, device, base_progress=0):
        with torch.no_grad():
            ...
            for i, (dark_images, light_images, target) in enumerate(loader):
                i = base_progress + i
                ...
                acc1, acc5 = accuracy(cls_output, target, topk=(1, 5))
                # acc1, acc5 = (float('nan'), float('nan'))
                loss_CLS.update(loss["cls"].item(), dark_images.size(0))
                loss_MSE.update(loss["mse"].item(), dark_images.size(0))
                top1.update(acc1[0], dark_images.size(0))
                top5.update(acc5[0], dark_images.size(0))
    ...
    if args.distributed and (
        ...
        run_validate(aux_val_loader, device, len(val_loader))
